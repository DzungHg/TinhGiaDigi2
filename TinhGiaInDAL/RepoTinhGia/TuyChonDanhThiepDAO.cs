...
{
    public class TuyChonDanhThiepDAO : ITuyChonDanhThiepDAO
    {
        QuanLyGiaInDBContext db = new QuanLyGiaInDBContext();

        public IEnumerable<TuyChonDanhThiepBDO> DocTatCa()
        {
            List<TuyChonDanhThiepBDO> list = null;
            try
            {
                var nguon = db.TUY_CHON_DANH_THIEP.Select(x => new TuyChonDanhThiepBDO
                {
                    ID = x.ID,                   
                    Ten = x.ten,                                   
                    ThuTu = (int)x.thu_tu
        ...
        {
            TuyChonDanhThiepBDO giay = null;
            try
            {
                var nguon = db.TUY_CHON_DANH_THIEP.Where(x => x.ID == iD).Select(x => new TuyChonDanhThiepBDO
                {
                    ID = x.ID,
                    Ten = x.ten,                 
                    ThuTu = (int)x.thu_tu
        ...
        #endregion
        private void ChuyenBDOThanhDAO(TuyChonDanhThiepBDO entityBDO, TUY_CHON_DANH_THIEP entityDAO)
        {
